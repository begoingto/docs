<template>
	<div>
		...
		<main>
			...
			>
				<div
					v-if="learning.excerpt"
					class="w-[300px] p-5 sticky top-3 border rounded-md bg-white hidden lg:block"
				>
					<h2 class="text-sm font-bold mb-4 font-kh">តារាងមាតិកា</h2>
					<ul class="space-y-2">
				...
				<ClientOnly>
					<ContentRenderer
						class="prose prose-sm prose-slate learning-link pr-7 max-w-none"
						:value="learning"
					>
						<template #empty>
...
<script setup>
	...
	const { data: learning } = await useAsyncData(slug, () => {
		return queryContent(slug).findOne();
	});

	const toc = computed(() => {
		if (!learning.value) return [];
		const items = learning.value.excerpt?.children;
		if (!items) return [];
		const toc = [];
